...
export function useDMs(token: string, currentUserId: number) {
  ...
  const openDM = useCallback(async (userId: number): Promise<string | null> => {
    try {
  ...
  const markRead = useCallback(async (channelId: string) => {
    try {
      ...
      setConversations(prev =>
        prev.map(c => c.channelId === channelId ? { ...c, unread_count: 0 } : c)
      );
    } catch {
      // non-fatal
    }
  ...
  const onDMMessage = useCallback((channelId: string, content: string, senderId: number, createdAt: string) => {
    setConversations(prev => {
        ...
        return prev.map(c =>
          c.channelId === channelId
            ? {
                ...
                unread_count: senderId !== currentUserId ? c.unread_count + 1 : c.unread_count,
              }
            : c
        ).sort((a, b) =>
  ...
  return { conversations, loading, openDM, markRead, onDMMessage, fetchConversations, totalUnread };
}